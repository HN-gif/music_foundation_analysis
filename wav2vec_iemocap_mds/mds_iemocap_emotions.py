...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract layer-wise wav2vec2 embeddings from IEMOCAP utterances, "
    ...
    parser.add_argument(
        ...
        default=["all", "genders"],
        help="Analysis group scopes to run.",
    )
    parser.add_argument(
        "--pair-sample-size",
        type=int,
        default=None,
        help="Maximum vector-pair count sampled per emotion pair for method C.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of MDS dimensions. Use 3 for a 3D MDS embedding.",
    )
    parser.add_argument("--sessions", nargs="+", default=None)
    parser.add_argument("--emotion-labels", nargs="+", default=None)
    parser.add_argument("--dialog-types", nargs="+", choices=["impro", "script"], default=None)
    parser.add_argument(
        "--include-xxx",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-groups-with-fewer-emotions",
        type=int,
        default=2,
        help="Skip groups with fewer than this many emotion labels.",
    )
...
def method_b(
    ...
    return build_distance_matrix(representative_matrix, ordered_labels, metric), used_speaker_emotion_groups, notes
...
def run_mds(
    ...
    if n_components < 1:
        raise ValueError("--mds-components must be at least 1.")
    mds = MDS(
...
def run_method(
    ...
    if method == "A":
        ...
        distance_matrix, sample_count, notes = method_c(
...
def plot_mds(
    ...
    coordinate_dims = sorted(
        [key for key in coordinates[0] if key.startswith("mds_")],
    ...
    if plot_dims >= 3:
        ...
        for row in coordinates:
            ...
            display_label = f"{label} ({row['emotion_name']})"
            x_value = float(row["mds_1"])
            y_value = float(row["mds_2"])
            z_value = float(row["mds_3"])
            ax.scatter(
                x_value,
                y_value,
                z_value,
            ...
            ax.text(x_value, y_value, z_value, display_label, fontsize=9, fontweight="bold")
        ax.set_zlabel("MDS-3")
    else:
    ...
    title = (
        f"{group.scope}: {group.group_id}\n"
        f"Layer {layer_index:02d} Method {method} IEMOCAP emotion MDS "
        f"({len(coordinate_dims)}D; plotting first {plot_dims}D)\n"
        f"stress={stress:.6g}"
    )
    ax.set_title(title)
    ax.set_xlabel("MDS-1")
...
def coordinate_fieldnames(coordinates: Sequence[Dict[str, float | str]]) -> List[str]:
    ...
    return ["emotion_label", "emotion_name", *coordinate_dims]
...
def save_method_outputs(
    ...
    write_dict_rows(
        coordinates_path,
        result.coordinates,
        coordinate_fieldnames(result.coordinates),
    )
...
def build_group_sample_count_rows(
    ...
    for group in groups:
        ...
        analyzed = emotion_count >= minimum_emotions
        row: Dict[str, object] = {
            "group_scope": group.scope,
            "group_id": group.group_id,
        ...
        for label in EMOTION_ORDER:
            if label in EXCLUDED_EMOTION_LABELS:
                continue
...
def main() -> None:
    ...
    all_embeddings = build_embeddings(utterances, args.model_name, args.device)
    ...
    group_count_rows = build_group_sample_count_rows(
        groups,
        all_embeddings,
        args.skip_groups_with_fewer_emotions,
    )
    write_dict_rows(
    ...
    for group in groups:
        ...
        if len(ordered_labels) < args.skip_groups_with_fewer_emotions:
            print(
                f"Skipping {group.scope} {group.group_id}: "
                f"only {len(ordered_labels)} emotion labels ({summarize_group_labels(embeddings)})"
            )
            continue

        print(
            f"Analyzing {group.scope} {group.group_id}: "
            f"{len(embeddings)} utterances, labels {summarize_group_labels(embeddings)}"
        )
